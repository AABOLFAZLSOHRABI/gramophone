...
public:
  using PlatformThreadDispatcher = std::function<void(std::function<void()>)>;
  ...
  JustAudioEventSink::JustAudioEventSink(
  ...
  ) : dispatcher_(std::move(dispatcher)), sink_holder_(std::make_shared<SinkHolder>()) {
    auto event_channel =
    ...
    auto event_handler = std::make_unique<flutter::StreamHandlerFunctions<>>(
      ...
      std::lock_guard<std::mutex> lock(self->sink_holder_->mutex);
      self->sink_holder_->sink = std::move(events);
      return nullptr;
    }, [self = this](const EncodableValue* arguments) -> std::unique_ptr<flutter::StreamHandlerError<>> {
      std::lock_guard<std::mutex> lock(self->sink_holder_->mutex);
      self->sink_holder_->sink.reset();
      return nullptr;
    });
  ...
  void Success(const EncodableValue& event) {
    auto holder = sink_holder_;
    auto copied_event = event;
    dispatchToPlatformThread([holder, copied_event]() {
      std::lock_guard<std::mutex> lock(holder->mutex);
      if (holder->sink) {
        holder->sink->Success(copied_event);
      }
    });
  }
  ...
  void Error(const std::string& error_code,
    ...
    auto holder = sink_holder_;
    auto copied_code = error_code;
    auto copied_message = error_message;
    dispatchToPlatformThread([holder, copied_code, copied_message]() {
      std::lock_guard<std::mutex> lock(holder->mutex);
      if (holder->sink) {
        holder->sink->Error(copied_code, copied_message);
      }
    });
  }
private:
  void dispatchToPlatformThread(std::function<void()> action) {
    if (dispatcher_) {
      dispatcher_(std::move(action));
      return;
    }
    action();
  }
  ...
  std::shared_ptr<SinkHolder> sink_holder_;
};
...
public:
  ...
  AudioPlayer::AudioPlayer(
    std::string idx,
    flutter::BinaryMessenger* messenger,
    JustAudioEventSink::PlatformThreadDispatcher dispatcher
  ) {
    id = idx;
